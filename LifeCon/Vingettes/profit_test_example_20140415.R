...
# Reserve Basis
ResBasis <- list()
ResBasis$interest <- function(t=0, ...) {0.04}
ResBasis$qxt <- seq(0.011, 0.020, 0.001)
# ResBasis
...
ResPP <- function(x=0, Res.t=0, PSD=ProjStartDate, PB=PolBasis,
	RB=ResBasis) {
	out <- NULL
	tempDate <- PSD + Res.t
	PolDur <- .GetPolDur(Date=tempDate,
	...
	if({PolDur<0} || {PolDur>PB$TermDur}) {
		out <- 0
			...
			temp <- c(ResBenFactors*Benefits$death() -
				ResBenPrem*ResPremFactors,0)
			out <- temp[Res.t+1] # TODO needs to be change for m-thly
		}	
	return(out)
}
ResPP(Res.t=0)
...
for (j in 1:length(cf.data$time)) {
	...
	if(t==0) {
		...
		temp$PrPPtm1.t <- - Expense.init(prem=gp)
			...
			temp$ResPPtm1 <- ResPP(Res.t=prev.t)
			temp$PremPPtm1 <- gp
			...
			temp$ResPPtm1.t <- ResPP(Res.t=t)*(1 - exp.qxt(t-1))
			temp$PrPPtm1.t <- (
				temp$ResPPtm1 + 
				temp$PremPPtm1 -
				temp$RenewExpensePPtm1 +
